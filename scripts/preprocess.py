...
class Preprocess:
    ...
    def get_num_columns(self, df):
        """Get columns with numerical value."""
        try:
            self.logger.info('Getting columns with numerical value...')
            ...
            self.logger.exception(
                'Failed to get columns with numerical value')
            sys.exit(1)

    def get_cat_columns(self, df):
        """ Get columns with categorical value."""
        try:
            self.logger.info('Getting columns with categorical value....')
            return df.select_dtypes(
                include="object").columns.tolist()
        except Exception:
            self.logger.exception(
                'Failed to get columns with categorical value')
            sys.exit(1)
    ...
    def fill_median(self, df, columns):
    ...
    def fill_with_zero(self, df, columns):
    ...
    def join_df(self, df1, df2, on, how="inner"):
        """Join two dataframes."""
        try:
